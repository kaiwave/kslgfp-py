...
def run_quiz(cards):
    ...
    for i, card in enumerate(cards, 1):  # actual game loop
        try:
            print(
                f"[{i}/{total}] [{card['type'].upper()}] {card['prompt'].split('-', 1)[1]}")  # for new conjugation endings feature/inline prompt hints
...
def main():
    ...
    selected_cards = choose_type(card_types, all_cards)
